    ...
    <Tab.Navigator
      screenOptions={({route}) => ({
        tabBarIcon: ({color, size}) => {
          if (route.name === MAIN) {
            return <Icn_home size={size} fill={color} />;
          } else if (route.name === COMMENTS) {
            return <Icn_revievs size={size} stroke={color} />;
          } else if (route.name === QR_CODE) {
            return <Icn_qrcode size={size} stroke={color} fill={color} />;
          } else if (route.name === TIPS) {
            return <Icn_tips size={size} fill={color} />;
          } else if (route.name === SETTINGS_NAVIGATOR) {
            return <Icn_settings size={size} fill={color} />;
          }
        },
      })}
      tabBarOptions={{
